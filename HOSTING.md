...
2. **`groupId` decision.** Jenkins's current style guide states: *"All new hosting requests are
   instructed to use `io.jenkins.plugins` as group ID."* This repo currently uses
   `io.github.retrokharkov1`. Other group IDs aren't strictly forbidden, but `io.jenkins.plugins`
   is what new requests are steered toward, and **the groupId is effectively locked in once the
   first hosted release goes out** (changing it afterward means a new release coordinate, breaking
   any existing installs' upgrade path). This is a one-way door — decide before filing the hosting
   request, not after.
3. **No `Jenkinsfile` yet.** Not required to file the hosting request itself, but required before
   ...
   transfer.
4. **GitHub two-factor authentication (2FA).** The hosting-request docs fetched above do not
   ...
   invitation later.
5. **Jenkins community account.** A Jenkins account (via <https://www.jenkins.io/sso/>) is
...
| **New Repository Name** | The name it will have inside `jenkinsci`. Plugin repos must end in `-plugin` and be lowercase, so this should be `config-template-sync-plugin` (confirm the exact name against the current naming convention page before submitting — it may have evolved). |
...
- **Repository transfer:** Once approved, the existing repository is **forked into the
  ...
  `github.com/jenkinsci/config-template-sync-plugin` (or whatever name was requested), not at
...
- **CI enablement:** `ci.jenkins.io` builds are wired up next, driven by a `Jenkinsfile` in the
  repo (see the still-open item #3 above) — this is the point at which that file becomes
...
automation in this repo can file that request on the owner's behalf. Sections 1 and the still-open
items are things an implementer *could* mechanically do (bump the parent POM, decide and change
the `groupId`, add a `Jenkinsfile`) — but the `groupId` choice in particular is a one-way decision
tied to filing this same request, so it is listed here as something to decide, not something
already done.
