...
/**
 * One entry in an env {@link ConfigSetVersion}'s ordered {@code baseChain} (FR-51/FR-55): operator
 * *intent* at save time — which base Config Set, and whether to always track its active version or
 * pin to one specific historical version. Order within the owning list is significant (FR-8: later
...
public final class BaseConfigReference implements Serializable {
    ...
    public BaseConfigReference(String projectKey, PinMode pinMode, int pinnedVersionNumber) {
        ...
        // ACTIVE carries no meaningful pinned number (FR-51) — normalize to 0 so equals()/hashCode()
    ...
    /** Human-readable form used in FR-26's per-reference fallback log line and FR-58's history display. */
    @Override
