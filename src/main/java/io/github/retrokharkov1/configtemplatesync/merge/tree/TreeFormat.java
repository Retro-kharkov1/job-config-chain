...
/**
 * One content type's parse/serialize contract (FR-63-FR-66). Registered per {@link ContentType} via
 * {@link TreeFormats#forType(ContentType)} — the single dispatch point every content-reading call
 * site in this plugin MUST go through (FR-66), never a hardcoded Gson/DOM/SnakeYAML call outside
 * this package.
 */
public interface TreeFormat {
    ...
    /** Pretty-printed, canonical serialization for this format — the ONLY serialization path (FR-64). */
